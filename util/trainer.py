    ...
    def train(self, rng, dataset, loss_fn, initial_params):
        ...
        def iter_fn(key, opt_state, vars, batch, iteration):
            ...
            def avg_loss(params, key):
                arg_vars = frozen_dict.freeze(
                    {'params': params, 'batch_stats': batch_stats}) if 'batch_stats' in vars else \
                    frozen_dict.freeze({'params': params})
                keys = jax.random.split(key, self.batch_size)
                batch_loss, stats, new_batch_stats = jax.vmap(loss_fn, in_axes=(0, None, 0, None),
                                                              out_axes=(0, 0, None), axis_name='batch')(keys, arg_vars,
                                                                                                        batch,
                                                                                                        iteration)
            ...
            new_vars = frozen_dict.freeze(
                {'params': new_params, 'batch_stats': new_batch_stats['batch_stats']}) if 'batch_stats' in vars else \
                ...
                with tqdm.tqdm(total=self.epochs) as pbar:
                    for epoch in range(self.epochs):
                        ...
                        with tqdm.tqdm(total=len(data), leave=False) as ibar:
                            ...
                            _, opt_state, params, iteration = train_epoch(next(rng_seq), iterator, opt_state, params,
                                                                          iteration)
                        stats_str = ' '.join(
                            ['{}: {: <12g}'.format(k, v) for (k, v) in stats.items()]) if stats is not None else ''
